...
- Persisted data is exactly the files in spec §5 (`season.yaml`, `groups/*.yaml`, `preopening_projections.yaml`, `movies_overrides.yaml`, `box_office_history.jsonl`, `forecast_history.jsonl`). There is deliberately **no persisted refresh/run-date record**: a degraded production refresh appends nothing (§5.6), and a refresh date that is consequently absent from the odds-over-time axis is accepted behaviour, not a defect. Reviewers must not request refresh-date persistence.
